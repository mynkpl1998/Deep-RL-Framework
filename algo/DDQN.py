...
class DDQN():
    ...
    def update(self):
        ...
        '''
        #------ Double DQN Update Starts ------#
        '''

        # Get maximizing action for next state using primary q_network
        Q_next = self.main_model.forward(next_states_torch, bsize=self.params["batch_size"]).detach()
        Q_next_max_acts = torch.argmax(Q_next,dim=1).unsqueeze(dim=1)

        # Calculate target values
        Q_next_target = self.target_model.forward(next_states_torch, bsize=self.params["batch_size"]).detach()
        next_state_values = torch.mul(Q_next_target.gather(index=Q_next_max_acts,dim=1).squeeze(dim=1),game_over_torch)
        target_values = rewards_torch + ( self.params["gamma"] * next_state_values)

        '''
        #------ Double DQN Update Ends -------#
        '''
